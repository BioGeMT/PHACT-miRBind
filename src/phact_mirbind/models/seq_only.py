"""Seq-only pairwise miRBind CNN."""

from __future__ import annotations

import torch
import torch.nn as nn
import torch.nn.functional as F

from phact_mirbind.models.blocks import (
    make_conv_stack,
    run_conv_stack,
    validate_conv_config,
)


class PairwiseSeqCNN(nn.Module):
    """Seq-only miRBind pairwise CNN using the original pair grid."""

    def __init__(
        self,
        *,
        num_pair_classes: int,
        target_length: int = 50,
        mirna_length: int = 28,
        embedding_dim: int = 8,
        dropout_rate: float = 0.2,
        filter_sizes: tuple[int, ...] = (128, 64, 32),
        kernel_sizes: tuple[int, ...] = (6, 3, 3),
    ) -> None:
        super().__init__()
        validate_conv_config(filter_sizes, kernel_sizes)

        self.num_pair_classes = num_pair_classes
        self.target_length = target_length
        self.mirna_length = mirna_length
        self.embedding_dim = embedding_dim

        self.pair_linear = nn.Linear(num_pair_classes, embedding_dim)
        (
            self.conv_layers,
            self.bn_layers,
            self.pool_layers,
            self.dropout_layers,
        ) = make_conv_stack(
            in_channels=embedding_dim,
            filter_sizes=filter_sizes,
            kernel_sizes=kernel_sizes,
            dropout_rate=dropout_rate,
        )

        self.flat_features = self._compute_flat_features()
        self.fc1 = nn.Linear(self.flat_features, 30)
        self.bn_fc = nn.BatchNorm1d(30)
        self.dropout_fc = nn.Dropout(dropout_rate)
        self.fc2 = nn.Linear(30, 1)

    def _compute_flat_features(self) -> int:
        pair_indices = torch.zeros(1, self.mirna_length, self.target_length).long()
        x = self._build_conv_input(pair_indices)
        x = run_conv_stack(x, self.conv_layers, self.bn_layers, self.pool_layers)
        return x.numel()

    def forward(self, pair_indices: torch.Tensor) -> torch.Tensor:
        features = self.extract_features(pair_indices)
        return self.classify_features(features)

    def extract_features(self, pair_indices: torch.Tensor) -> torch.Tensor:
        x = self._build_conv_input(pair_indices)
        x = run_conv_stack(
            x,
            self.conv_layers,
            self.bn_layers,
            self.pool_layers,
            self.dropout_layers,
        )
        return x.contiguous().view(x.size(0), -1)

    def classify_features(self, features: torch.Tensor) -> torch.Tensor:
        x = self.dropout_fc(self.project_features(features))
        return self.fc2(x).squeeze(-1)

    def project_features(self, features: torch.Tensor) -> torch.Tensor:
        """Project flattened convolutional features into the 30-value latent."""
        return F.leaky_relu(self.bn_fc(self.fc1(features)), 0.1)

    def encode(self, pair_indices: torch.Tensor) -> torch.Tensor:
        """Return the latent used immediately before the final classifier."""
        return self.project_features(self.extract_features(pair_indices))

    def predict_proba(self, pair_indices: torch.Tensor) -> torch.Tensor:
        return torch.sigmoid(self.forward(pair_indices))

    def _build_conv_input(self, pair_indices: torch.Tensor) -> torch.Tensor:
        pair_onehot = F.one_hot(
            pair_indices.long(),
            num_classes=self.num_pair_classes,
        ).float()
        return self.pair_linear(pair_onehot).permute(0, 3, 1, 2)
