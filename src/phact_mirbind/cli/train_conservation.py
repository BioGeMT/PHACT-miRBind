"""Training CLI for seq + phyloP/phastCons miRBind."""
...
def main() -> None:
    ...
    set_seed(args.seed)
    conservation_features = parse_conservation_features(args.conservation_features)
    ...
    model_params = {
        ...
        "conservation_channel_count": train_dataset.conservation_channel_count,
    ...
    model = PairwiseConservationCNN(**model_params).to(device)
    summary = {
        "model_type": "pairwise_seq_conservation",
        "model_params": json_ready(model_params),
        "conservation_features": list(conservation_features),
        "seed": args.seed,
    ...
    print(f"Conservation features: {','.join(conservation_features)}")
    train_model(
        model=model,
        loaders=loaders,
        ...
        checkpoint_prefix="pairwise_conservation_model",
        summary=summary,
        num_epochs=args.num_epochs,
        patience=args.patience,
        progress_every=args.progress_every,
        show_progress=args.progress_bar,
        extra_checkpoint={"conservation_features": list(conservation_features)},
    )


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train miRBind CNN with phyloP/phastCons target channels"
    )
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("outputs/conservation"))
    parser.add_argument(
...
def dataset(
    ...
    shuffle_mode: str,
    seed: int,
    conservation_features: tuple[str, ...],
) -> CachedConservationPairwiseIterableDataset:
    return CachedConservationPairwiseIterableDataset(
        cache_path,
...
def loader(
    dataset_obj: CachedConservationPairwiseIterableDataset,
    batch_size: int,
    num_workers: int,
    device,
) -> DataLoader:
    return DataLoader(
        ...
        collate_fn=make_conservation_collate(dataset_obj.mirna_length),
    )
