...
| Fifty-model comparison | `analyses/supporting/reproduction/final_analysis/phact_final_builder.py` | Metric/prediction sources identified in `analyses/final/all_model_test_leftout_auprc.tsv`; baseline, experiment and Agentomics result directories | Final comparison TSV/CSV/Markdown and figures in `analyses/final/` |
| P1 profile and subgroup evidence | `analyses/supporting/reproduction/followup_2026_09_06/analysis/analyze_param1_evidence.py` | Old and P1 arm score tables, P1 evaluation caches, benchmark labels, baseline and release predictions | All files in `analyses/final/p1_evidence/`; the retained report used 1,000 cluster-bootstrap replicates |
| Variant and miRNA profile figures | `analyses/supporting/reproduction/final_analysis/phact_variant_groundtruth_audit.py`, `phact_variant_validation_plot.py`, `phact_final_builder.py` | `data/inputs/score_import_20260813/`, `data/inputs/consensus_tree_scores.zip`, `data/inputs/reference/` | Variant/profile plot data, figures and audit records in `analyses/final/` |
| Fixed positional-control study | `analyses/supporting/reproduction/followup_2026_09_06/{controls,training}/` | P1 cache, fixed control sidecars, conservation cache, frozen package, protocol and original driver hashes | Entire `runs/followup-2026-09-06/`, especially all 18 checkpoint/result/prediction sets and aggregate tables |
| Exposure, subgroup and pooled-AP analyses | `analyses/supporting/reproduction/followup_2026_09_06/analysis/` | Benchmark TSVs, old/P1 caches and retained predictions; exact consumed files are listed in the two input manifests | Joined predictions, tables, audits and input manifests in the follow-up's `analysis/` directory |
