...
position_score <- function(ps, x, msa, num_nodes, num_leaves, total_pos, human_plc, node_human, nodes_raxml, human_leaf_len, dist_node, dist_leaf, parameter, leaves_conn, nodes_conn, chosen_leaves, chosen_nodes2, d_n, d_l) {
  ...
  matrix_prob <- matrix_prob[nodes_raxml, , drop = FALSE]
  # Calculate confident node count
  if (nrow(matrix_prob) > 1) {
    # The normal case: more than one node
    max_probs_per_node <- apply(matrix_prob, 1, max)
    confident_node_count <- sum(max_probs_per_node >= 0.5)
  } else if (nrow(matrix_prob) == 1) {
    # Special case: exactly one node
    max_probs_per_node <- max(matrix_prob)
    confident_node_count <- sum(max_probs_per_node >= 0.5)
  } else {
    # Special case: zero nodes
    max_probs_per_node <- numeric(0) # An empty numeric vector
    confident_node_count <- 0
  }
  ...
  human_nt_count <- sum(position_num == nt_f)
  scores <- list()

  # RAW SCORES
  scores$score_with_leaf <- score/human_nt_count
  scores$score_without_leaf <- score_without_leaf/human_nt_count
  
  # TRANSFORMED SCORES (0-1)
  #scores$score_with_leaf <- 1- log((score/human_nt_count)+10^(-10))/log(10^(-10))
  #scores$score_without_leaf <- 1- log((score_without_leaf/human_nt_count) + 10^(-15))/log(10^(-15))
