#!/usr/bin/env python3
"""Train a single miRBind2-sequence + PHACT/context fusion model.

The supplementary miRBind2 initialization checkpoint is supplied explicitly
with ``--mirbind2-checkpoint``. All preprocessing statistics are fit on
``--train-data`` only and saved into ``artifacts-dir/model.pt``.
"""
...
MIRBIND2_CHECKPOINT_PATH: Path | None = None
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train miRBind2+PHACT fusion model")
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Pretrained miRBind2 sequence-CNN checkpoint used for initialization.",
    )
...
def load_pretrained_sequence(model: MiRBind2PhactFusionNet, device: torch.device) -> Dict[str, object]:
    ckpt_path = MIRBIND2_CHECKPOINT_PATH
    if ckpt_path is None:
        ckpt_path = Path(__file__).resolve().parent / MIRBIND2_CHECKPOINT_FILENAME
    summary: Dict[str, object] = {
    ...
    if not ckpt_path.exists():
        summary["error"] = "local checkpoint copy not found"
        return summary
    try:
        checkpoint = torch.load(ckpt_path, map_location=device, weights_only=False)
        summary["checkpoint_top_level_keys"] = sorted(list(checkpoint.keys()))
        summary["checkpoint_model_params"] = checkpoint.get("model_params")
        state = checkpoint["model_state_dict"]
        model.seq_encoder.load_state_dict(state, strict=True)
        summary["pretrained_loaded"] = True
        summary["strict_load"] = True
        summary["checkpoint_epoch"] = int(checkpoint.get("epoch", -1)) if checkpoint.get("epoch", None) is not None else None
        if "val_auprc" in checkpoint:
            summary["checkpoint_val_auprc"] = float(checkpoint["val_auprc"])
    except Exception as e:
...
def make_loader(dataset: Dataset, batch_size: int, shuffle: bool, num_workers: int, drop_last: bool = False) -> DataLoader:
    # Keep workers at zero unless explicitly changed by a very conservative shm check.
    shm_avail = 0
    try:
        st = os.statvfs("/dev/shm")
        shm_avail = int(st.f_bavail * st.f_frsize)
    except Exception:
        pass
    if shm_avail and shm_avail < 2 * 1024 * 1024 * 1024:
        num_workers = 0
...
def compute_metrics(y_true: np.ndarray, prob: np.ndarray) -> Dict[str, float]:
    ...
    metrics["auprc"] = float(average_precision_score(y_true, prob)) if len(np.unique(y_true)) > 1 else float("nan")
    metrics["auroc"] = float(roc_auc_score(y_true, prob)) if len(np.unique(y_true)) > 1 else float("nan")
    metrics["log_loss"] = float(log_loss(y_true, prob, labels=[0, 1]))
    metrics["brier"] = float(brier_score_loss(y_true, prob))
    metrics["accuracy_0p5"] = float(accuracy_score(y_true, pred))
    metrics["precision_0p5"] = float(precision_score(y_true, pred, zero_division=0))
    metrics["recall_0p5"] = float(recall_score(y_true, pred, zero_division=0))
    metrics["f1_0p5"] = float(f1_score(y_true, pred, zero_division=0))
    metrics["mcc_0p5"] = float(matthews_corrcoef(y_true, pred))
    return metrics


def unpack_batch(batch, device: torch.device, x_dtype: torch.dtype):
    mirna, target, aux, target_tensor, mirna_phact, feat, dom, rinalmo_mirna, rinalmo_target, y = batch
    ...
    return pairwise, rc_pairwise, aux, target_tensor, mirna_phact, feat, dom, rinalmo_mirna, rinalmo_target, y


@torch.no_grad()
def evaluate_model(model: nn.Module, loader: DataLoader, criterion: nn.Module, device: torch.device, use_amp: bool) -> Tuple[np.ndarray, np.ndarray, float]:
    model.eval()
    ...
    for batch in loader:
        pairwise, rc_pairwise, aux, target_tensor, mirna_phact, feat, dom, rinalmo_mirna, rinalmo_target, y = unpack_batch(batch, device, x_dtype)
        with torch.amp.autocast(device_type=device.type, enabled=(device.type == "cuda" and use_amp)):
        ...
        n = y.shape[0]
        total_loss += float(loss.item()) * int(n)
        total_n += int(n)
    if total_n == 0:
        raise ValueError("Evaluation loader produced no samples")
    return np.concatenate(labels), np.concatenate(probs), total_loss / total_n


def train_one_epoch(
    model: MiRBind2PhactFusionNet,
    loader: DataLoader,
    criterion: nn.Module,
    ...
    model.train()
    if model.sequence_frozen:
        model.seq_encoder.eval()
    total_loss = 0.0
    total_n = 0
    x_dtype = torch.float16 if (device.type == "cuda" and use_amp) else torch.float32
...
def choose_batch_size(device: torch.device, use_amp: bool, cfg: ArchitectureConfig, pretrained_summary: Dict[str, object]) -> int:
    if device.type != "cuda":
        return 512
    for bs in BATCH_SIZE_CANDIDATES:
        try:
            model = MiRBind2PhactFusionNet(cfg).to(device)
            if pretrained_summary.get("pretrained_loaded"):
                load_pretrained_sequence(model, device)
            ...
            mirna = torch.zeros((bs, MIRNA_LENGTH), dtype=torch.uint8, device=device)
            target = torch.zeros((bs, TARGET_LENGTH), dtype=torch.uint8, device=device)
            x_dtype = torch.float16 if (device.type == "cuda" and use_amp) else torch.float32
            aux = torch.zeros((bs, cfg.aux_numeric_dim), dtype=x_dtype, device=device)
            target_tensor = torch.zeros((bs, TARGET_BRANCH_CHANNELS, TARGET_LENGTH), dtype=x_dtype, device=device)
            mirna_phact = torch.zeros((bs, MIRNA_PHACT_CHANNELS, MIRNA_LENGTH), dtype=x_dtype, device=device)
            feat = torch.zeros((bs,), dtype=torch.long, device=device)
            dom = torch.zeros((bs,), dtype=torch.long, device=device)
            rinalmo_mirna = torch.zeros((bs, cfg.rinalmo_hidden_size), dtype=x_dtype, device=device)
            rinalmo_target = torch.zeros((bs, cfg.rinalmo_hidden_size), dtype=x_dtype, device=device)
            y = torch.zeros((bs,), dtype=torch.float32, device=device)
            pairwise = build_mirbind2_onehot(mirna, target, dtype=x_dtype)
            rc_pairwise = build_rc_pairwise_grid(mirna, target, dtype=x_dtype)
            ...
            scaler.step(opt)
            scaler.update()
            del model, criterion, opt, scaler, mirna, target, aux, target_tensor, mirna_phact, feat, dom, rinalmo_mirna, rinalmo_target, y, pairwise, rc_pairwise, logits, loss
            torch.cuda.empty_cache()
            gc.collect()
            return bs
        except RuntimeError as e:
...
def representation_smoke_test(dataset: FusionDataset, cfg: ArchitectureConfig, device: torch.device, use_amp: bool, pretrained_summary: Dict[str, object]) -> Dict[str, object]:
    n = len(dataset)
    if n == 0:
    ...
    deterministic = all(np.array_equal(np.asarray(a), np.asarray(b)) for a, b in zip(sample0, sample0_repeat))
    finite_first = {
        "aux_numeric": bool(np.isfinite(dataset.aux_numeric[: min(1024, n)]).all()),
        "target_tensor": bool(np.isfinite(dataset.target_tensor[: min(1024, n)]).all()),
        "mirna_phact_tensor": bool(np.isfinite(dataset.mirna_phact_tensor[: min(1024, n)]).all()),
        "rinalmo_mirna_embedding": bool(np.isfinite(dataset.rinalmo_mirna_embedding[: min(1024, n)]).all()),
        "rinalmo_target_embedding": bool(np.isfinite(dataset.rinalmo_target_embedding[: min(1024, n)]).all()),
    }
    model = MiRBind2PhactFusionNet(cfg).to(device)
    if pretrained_summary.get("pretrained_loaded"):
        load_pretrained_sequence(model, device)
    model.eval()
    loader = make_loader(Subset(dataset, list(range(check_n))), batch_size=check_n, shuffle=False, num_workers=0)
    batch = next(iter(loader))
    x_dtype = torch.float16 if (device.type == "cuda" and use_amp) else torch.float32
    criterion = nn.BCEWithLogitsLoss()
    with torch.no_grad(), torch.amp.autocast(device_type=device.type, enabled=(device.type == "cuda" and use_amp)):
        pairwise, rc_pairwise, aux, target_tensor, mirna_phact, feat, dom, rinalmo_mirna, rinalmo_target, y = unpack_batch(batch, device, x_dtype)
        logits = model(pairwise, rc_pairwise, aux, target_tensor, mirna_phact, feat, dom, rinalmo_mirna, rinalmo_target)
        loss = criterion(logits.float(), y)
    smoke = {
        "row_count": int(n),
        "checked_batch_size": int(check_n),
        "mirna_codes_shape": list(dataset.mirna_codes.shape),
        "target_codes_shape": list(dataset.target_codes.shape),
        "aux_numeric_shape": list(dataset.aux_numeric.shape),
        "target_tensor_shape": list(dataset.target_tensor.shape),
        "mirna_phact_tensor_shape": list(dataset.mirna_phact_tensor.shape),
        "feature_idx_shape": list(dataset.feature_idx.shape),
        "dominant_region_idx_shape": list(dataset.dominant_region_idx.shape),
        "rinalmo_mirna_embedding_shape": list(dataset.rinalmo_mirna_embedding.shape),
        "rinalmo_target_embedding_shape": list(dataset.rinalmo_target_embedding.shape),
        "finite_first_1024": finite_first,
        "deterministic_getitem": bool(deterministic),
        ...
        "pairwise_onehot_shape": list(pairwise.shape),
        "pairwise_onehot_channels": int(pairwise.shape[-1]),
        "rc_pairwise_grid_shape": list(rc_pairwise.shape),
        "rc_pairwise_channels": int(rc_pairwise.shape[1]),
        "expected_shapes_ok": bool(
            list(pairwise.shape[1:]) == [MIRNA_LENGTH, TARGET_LENGTH, MIRBIND2_ONEHOT_CHANNELS]
            and list(rc_pairwise.shape[1:]) == [RC_PAIRWISE_CHANNELS, MIRNA_LENGTH, TARGET_LENGTH]
            and list(dataset.target_tensor.shape[1:]) == [TARGET_BRANCH_CHANNELS, TARGET_LENGTH]
            and list(dataset.mirna_phact_tensor.shape[1:]) == [MIRNA_PHACT_CHANNELS, MIRNA_LENGTH]
            and dataset.aux_numeric.shape[1] == len(AUX_NUMERIC_FEATURE_NAMES)
            and dataset.rinalmo_mirna_embedding.shape[1] == cfg.rinalmo_hidden_size
            and dataset.rinalmo_target_embedding.shape[1] == cfg.rinalmo_hidden_size
        ),
    }
    del model, loader, batch, pairwise, rc_pairwise, aux, target_tensor, mirna_phact, feat, dom, rinalmo_mirna, rinalmo_target, y, logits, loss
    if device.type == "cuda":
        torch.cuda.empty_cache()
    gc.collect()
    return smoke
...
def write_validation_predictions(artifacts_dir: Path, ids: List[str], y_true: np.ndarray, prob: np.ndarray) -> None:
    out = pd.DataFrame(
        {
            "id": ids,
            "label": y_true.astype(np.int32),
            "prediction": (prob >= 0.5).astype(np.int32),
            "probability_0": 1.0 - prob,
            "probability_1": prob,
        }
    )
    out.to_csv(artifacts_dir / "validation_predictions.csv", index=False)


def write_epoch_metrics(artifacts_dir: Path, rows: List[Dict[str, object]]) -> None:
    if not rows:
        return
    keys: List[str] = []
    for row in rows:
        for key in row.keys():
            if key not in keys:
                keys.append(key)
    with open(artifacts_dir / "validation_metrics_by_epoch.csv", "w", newline="") as f:
...
def save_model_artifact(path: Path, model: nn.Module, arch_cfg: ArchitectureConfig, rep_cfg: RepresentationConfig, preprocessor: Dict[str, object], best_info: Dict[str, object]) -> None:
    artifact = {
        "state_dict": model.state_dict(),
        "architecture_config": asdict(arch_cfg),
        "representation_config": asdict(rep_cfg),
        "preprocessor": preprocessor,
        "training_info": best_info,
        "seed": SEED,
        "model_class": "MiRBind2PhactFusionNet",
        "pytorch_version": torch.__version__,
        "pretrained_loaded": bool(preprocessor.get("pretrained_summary", {}).get("pretrained_loaded", False)),
        "rinalmo_model_id": preprocessor.get("rinalmo_metadata", {}).get("model_id", RINALMO_MODEL_ID),
        "rinalmo_hidden_size": int(preprocessor.get("rinalmo_metadata", {}).get("hidden_size", RINALMO_HIDDEN_SIZE)),
        "rinalmo_metadata": preprocessor.get("rinalmo_metadata", {}),
    }
    torch.save(artifact, path / "model.pt")


def train_subset_metrics_from_loader(model: nn.Module, loader: DataLoader, criterion: nn.Module, device: torch.device, use_amp: bool, sample_count: int) -> Dict[str, float]:
    y, prob, loss = evaluate_model(model, loader, criterion, device, use_amp)
...
def is_better_checkpoint(new_info: Dict[str, object], best_info: Optional[Dict[str, object]]) -> bool:
    if best_info is None:
        return True
    new_metrics = new_info["validation_metrics"]
    best_metrics = best_info["validation_metrics"]
    new_auprc = float(new_metrics["auprc"])
    best_auprc = float(best_metrics["auprc"])
    if new_auprc > best_auprc + VALID_METRIC_EPS:
        return True
    if new_auprc < best_auprc - VALID_METRIC_EPS:
        return False
    new_logloss = float(new_metrics["log_loss"])
    best_logloss = float(best_metrics["log_loss"])
    if new_logloss < best_logloss - VALID_METRIC_EPS:
        return True
    if new_logloss > best_logloss + VALID_METRIC_EPS:
        return False
    # Final exact tie-break requested for this iteration: prefer fm_b only on exact AUPRC/log-loss tie.
    return str(new_info.get("candidate")) == "fm_b" and str(best_info.get("candidate")) != "fm_b"


def make_full_optimizer(model: MiRBind2PhactFusionNet, cand: Dict[str, object]) -> torch.optim.Optimizer:
    return torch.optim.AdamW(
        [
            {"params": model.sequence_body_parameters(), "lr": float(cand["body_lr"]), "name": "copied_mirbind2_body"},
            {"params": model.sequence_classifier_parameters(), "lr": float(cand["classifier_lr"]), "name": "copied_mirbind2_classifier"},
            {"params": model.new_parameters(), "lr": float(cand["new_lr"]), "name": "new_context_fusion"},
        ],
...
def evaluate_and_record(
    ...
    best_global_info: Optional[Dict[str, object]],
    patience_remaining: Optional[int],
    train_eval_sample_count: int,
) -> Tuple[Optional[Dict[str, object]], Dict[str, float]]:
    yv, prob, val_loss = evaluate_model(model, val_loader, criterion, device, use_amp)
    val_metrics = compute_metrics(yv, prob)
    val_metrics["loss"] = float(val_loss)
    train_metrics = train_subset_metrics_from_loader(model, train_eval_loader, criterion, device, use_amp, train_eval_sample_count)
    row: Dict[str, object] = {
        ...
        "sequence_body_lr": float(cand["body_lr"]),
        "sequence_classifier_lr": float(cand["classifier_lr"]),
        "new_lr": float(cand["new_lr"] if phase != "warmup" else cand["warmup_lr"]),
    }
    row.update({f"val_{k}": v for k, v in val_metrics.items()})
    row.update({f"train_subset_{k}": v for k, v in train_metrics.items()})
    epoch_rows.append(row)
    write_epoch_metrics(artifacts_dir, epoch_rows)
    reporter.report_epoch(
        epoch=global_epoch,
        train_loss=float(train_loss),
        val_loss=float(val_loss),
        val_metric_name="AUPRC",
        val_metric=float(val_metrics["auprc"]),
        early_stopping_patience_remaining=patience_remaining,
    )
    info: Dict[str, object] = {
        ...
        "sequence_body_lr": float(cand["body_lr"]),
        "sequence_classifier_lr": float(cand["classifier_lr"]),
        "new_lr": float(cand["new_lr"]),
        "warmup_lr": float(cand["warmup_lr"]),
        "validation_metrics": val_metrics,
        "validation_loss": float(val_loss),
        "train_loss": float(train_loss),
        "train_subset_metrics_at_epoch": train_metrics,
    }
    if is_better_checkpoint(info, best_global_info):
        save_model_artifact(artifacts_dir, model, arch_cfg, rep_cfg, preprocessor, info)
        write_validation_predictions(artifacts_dir, val_ids, y_val, prob)
        best_global_info = dict(info)
    return best_global_info, val_metrics
...
def train_candidate(
    ...
    rep_cfg: RepresentationConfig,
    preprocessor: Dict[str, object],
    pretrained_summary: Dict[str, object],
    epoch_rows: List[Dict[str, object]],
    reporter: TrainingReporter,
    ...
    set_seeds(SEED + candidate_index)
    model = MiRBind2PhactFusionNet(arch_cfg).to(device)
    actual_pretrained = load_pretrained_sequence(model, device)
    # Preserve the initial inspection result in metrics, but also keep per-candidate diagnostics.
    preprocessor.setdefault("candidate_pretrained_loads", {})[str(cand["name"])] = actual_pretrained
    ...
    optimizer = torch.optim.AdamW(model.new_parameters(), lr=float(cand["warmup_lr"]), weight_decay=WEIGHT_DECAY)
    scaler = torch.amp.GradScaler("cuda", enabled=(device.type == "cuda" and use_amp))

    global_epoch = global_epoch_start
    candidate_name = str(cand["name"])
    best_candidate_auprc = -float("inf")
    no_improve = 0

    # Warm-up epoch.
    global_epoch += 1
    train_loss = train_one_epoch(model, train_loader, criterion, optimizer, scaler, device, use_amp, reporter, global_epoch)
    best_global_info, val_metrics = evaluate_and_record(
        model,
        val_loader,
        train_eval_loader,
        criterion,
        ...
        train_eval_sample_count,
    )
    best_candidate_auprc = max(best_candidate_auprc, float(val_metrics["auprc"]))

    # Fine-tuning epochs.
    model.set_sequence_trainable(True)
    optimizer = make_full_optimizer(model, cand)
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode="max", factor=0.5, patience=1)
    scaler = torch.amp.GradScaler("cuda", enabled=(device.type == "cuda" and use_amp))
    for fine_epoch in range(1, FINE_TUNE_EPOCHS + 1):
        global_epoch += 1
        train_loss = train_one_epoch(model, train_loader, criterion, optimizer, scaler, device, use_amp, reporter, global_epoch)
        patience_remaining = max(0, EARLY_STOPPING_PATIENCE - no_improve)
        best_global_info, val_metrics = evaluate_and_record(
            model,
            val_loader,
            train_eval_loader,
        ...
        scheduler.step(float(val_metrics["auprc"]))
        if float(val_metrics["auprc"]) > best_candidate_auprc + VALID_METRIC_EPS:
            best_candidate_auprc = float(val_metrics["auprc"])
            no_improve = 0
        else:
            no_improve += 1
    ...
    if device.type == "cuda":
        torch.cuda.empty_cache()
    gc.collect()
    return best_global_info, global_epoch


# ------------------------------- main ---------------------------------

def main() -> None:
    global MIRBIND2_CHECKPOINT_PATH
    args = parse_args()
    MIRBIND2_CHECKPOINT_PATH = args.mirbind2_checkpoint.expanduser().resolve()
    if not MIRBIND2_CHECKPOINT_PATH.is_file():
        raise FileNotFoundError(MIRBIND2_CHECKPOINT_PATH)
    set_seeds(SEED)
    ...
    train_split_dir = Path(args.train_data)
    val_split_dir = Path(args.validation_data)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    use_amp = device.type == "cuda"
    num_workers = 0

    train_samples, y_train = load_split_frame(args.train_data, require_labels=True)
    val_samples, y_val = load_split_frame(args.validation_data, require_labels=True)
    assert y_train is not None and y_val is not None
    val_ids = val_samples["id"].tolist()
    train_ids = train_samples["id"].tolist()

    # Fit/transform PHACT tensors. Target-PHACT stats are fit while reading train only.
    train_target_phact, target_scalers, train_target_any, train_target_summary = fit_or_transform_target_phact(train_split_dir, train_ids, fit=True)
    val_target_phact, _, val_target_any, val_target_summary = fit_or_transform_target_phact(val_split_dir, val_ids, fit=False, scalers=target_scalers)
    train_mirna_phact, val_mirna_phact, train_mirna_aux, val_mirna_aux, mirna_metadata = fit_and_transform_mirna_phact(train_split_dir, train_ids, val_split_dir, val_ids)

    positional_scalers = fit_conservation_positional_scalers(train_samples)
    train_mirna_codes, train_target_codes, train_seq_features, train_cons_summary, train_cons_tensor = transform_core_arrays(train_samples, positional_scalers)
    val_mirna_codes, val_target_codes, val_seq_features, val_cons_summary, val_cons_tensor = transform_core_arrays(val_samples, positional_scalers)

    train_target_tensor = np.concatenate([train_cons_tensor, train_target_phact], axis=1).astype(np.float32, copy=False)
    val_target_tensor = np.concatenate([val_cons_tensor, val_target_phact], axis=1).astype(np.float32, copy=False)
    del train_cons_tensor, val_cons_tensor, train_target_phact, val_target_phact
    gc.collect()

    train_phact_aux = np.column_stack([train_mirna_aux, train_target_any.astype(np.float32)])
    val_phact_aux = np.column_stack([val_mirna_aux, val_target_any.astype(np.float32)])
    train_base_seq = train_seq_features[:, : len(BASE_SEQ_FEATURE_NAMES)]
    val_base_seq = val_seq_features[:, : len(BASE_SEQ_FEATURE_NAMES)]
    train_new_alignment = train_seq_features[:, len(BASE_SEQ_FEATURE_NAMES) :]
    val_new_alignment = val_seq_features[:, len(BASE_SEQ_FEATURE_NAMES) :]
    raw_aux_train = np.concatenate([train_base_seq, train_cons_summary, train_phact_aux.astype(np.float32), train_new_alignment], axis=1)
    raw_aux_val = np.concatenate([val_base_seq, val_cons_summary, val_phact_aux.astype(np.float32), val_new_alignment], axis=1)
    if raw_aux_train.shape[1] != len(AUX_NUMERIC_FEATURE_NAMES):
        raise ValueError(f"Aux feature shape mismatch: {raw_aux_train.shape[1]} vs {len(AUX_NUMERIC_FEATURE_NAMES)}")

    phact_metadata = dict(mirna_metadata)
    phact_metadata["target_phact_train_summary"] = train_target_summary
    phact_metadata["target_phact_validation_summary"] = val_target_summary
    pretrain_probe_model = MiRBind2PhactFusionNet(ArchitectureConfig())
    pretrained_summary = load_pretrained_sequence(pretrain_probe_model, torch.device("cpu"))
    del pretrain_probe_model
    gc.collect()

    # Frozen RiNALMo-micro embeddings are computed after row-order samples are loaded.
    # The scaler is fit on training embeddings only; validation is transformed with that scaler.
    optional_mini = train_split_dir.parent / "mini_train"
    train_rinalmo_mirna, train_rinalmo_target, val_rinalmo_mirna, val_rinalmo_target, rinalmo_metadata = build_rinalmo_embeddings_for_training(
        train_samples,
        val_samples,
        artifacts_dir,
        device,
        use_amp,
        optional_mini_data=optional_mini,
    )

    preprocessor = build_preprocessor(train_samples, phact_metadata, target_scalers, raw_aux_train, positional_scalers, pretrained_summary)
    preprocessor["rinalmo_metadata"] = rinalmo_metadata
    preprocessor["rinalmo_embedding_scaler"] = None
    aux_train = apply_standard_scaler(raw_aux_train, preprocessor["aux_numeric_scaler"])
    aux_val = apply_standard_scaler(raw_aux_val, preprocessor["aux_numeric_scaler"])

    feat_train = encode_categories(train_samples["feature"].tolist(), preprocessor["category_vocabs"]["feature"])
    feat_val = encode_categories(val_samples["feature"].tolist(), preprocessor["category_vocabs"]["feature"])
    dom_train = encode_categories(train_samples["dominant_region"].tolist(), preprocessor["category_vocabs"]["dominant_region"])
    dom_val = encode_categories(val_samples["dominant_region"].tolist(), preprocessor["category_vocabs"]["dominant_region"])

    arch_cfg = ArchitectureConfig(
        feature_vocab_size=len(preprocessor["category_vocabs"]["feature"]),
        dominant_region_vocab_size=len(preprocessor["category_vocabs"]["dominant_region"]),
        rinalmo_hidden_size=int(rinalmo_metadata.get("hidden_size", RINALMO_HIDDEN_SIZE)),
        rinalmo_concat_dim=int(rinalmo_metadata.get("concat_dim", RINALMO_CONCAT_DIM)),
    )
    rep_cfg = RepresentationConfig()
    ...
    # Free large intermediate arrays/dataframes that are no longer needed by training.
    n_pos = int(np.sum(y_train))
    n_neg = int(len(y_train) - n_pos)
    pos_rate = float(np.mean(y_train))
    del raw_aux_train, raw_aux_val, train_seq_features, val_seq_features, train_base_seq, val_base_seq, train_new_alignment, val_new_alignment, train_cons_summary, val_cons_summary
    del train_phact_aux, val_phact_aux, train_mirna_aux, val_mirna_aux, train_target_any, val_target_any
    del train_mirna_codes, train_target_codes, val_mirna_codes, val_target_codes, aux_train, aux_val
    del train_target_tensor, val_target_tensor, train_mirna_phact, val_mirna_phact, feat_train, feat_val, dom_train, dom_val
    del train_rinalmo_mirna, train_rinalmo_target, val_rinalmo_mirna, val_rinalmo_target
    del train_samples, val_samples
    gc.collect()
    if device.type == "cuda":
        torch.cuda.empty_cache()

    batch_size = choose_batch_size(device, use_amp, arch_cfg, pretrained_summary)
    criterion = nn.BCEWithLogitsLoss()

    smoke = representation_smoke_test(train_dataset, arch_cfg, device, use_amp, pretrained_summary)
    with open(artifacts_dir / "smoke_test.json", "w") as f:
        json.dump(smoke, f, indent=2)

    val_loader = make_loader(val_dataset, batch_size=batch_size * 2, shuffle=False, num_workers=num_workers)
    rng = np.random.default_rng(SEED)
    if len(train_dataset) > TRAIN_METRIC_SUBSET_SIZE:
        train_eval_idx = np.sort(rng.choice(len(train_dataset), size=TRAIN_METRIC_SUBSET_SIZE, replace=False)).tolist()
        train_eval_dataset: Dataset = Subset(train_dataset, train_eval_idx)
        train_eval_sample_count = TRAIN_METRIC_SUBSET_SIZE
    else:
        train_eval_dataset = train_dataset
        train_eval_sample_count = len(train_dataset)
    train_eval_loader = make_loader(train_eval_dataset, batch_size=batch_size * 2, shuffle=False, num_workers=num_workers)

    best_global_info: Optional[Dict[str, object]] = None
    epoch_rows: List[Dict[str, object]] = []
    global_epoch = 0
    for candidate_index, cand in enumerate(CANDIDATES):
        best_global_info, global_epoch = train_candidate(
            cand,
            candidate_index,
            train_dataset,
            val_loader,
    ...
    if best_global_info is None or not (artifacts_dir / "model.pt").exists():
        raise RuntimeError("No model checkpoint was saved; training did not complete any validation epoch")

    # Reload the selected checkpoint and compute final deterministic train-subset metrics.
    checkpoint = torch.load(artifacts_dir / "model.pt", map_location=device, weights_only=False)
    selected_arch = ArchitectureConfig(**checkpoint["architecture_config"])
    selected_model = MiRBind2PhactFusionNet(selected_arch).to(device)
    selected_model.load_state_dict(checkpoint["state_dict"])
    final_train_metrics = train_subset_metrics_from_loader(selected_model, train_eval_loader, criterion, device, use_amp, train_eval_sample_count)
    best_global_info["train_subset_metrics"] = final_train_metrics
    best_global_info["total_elapsed_seconds"] = time.time() - load_start
    best_global_info["device"] = str(device)
    if device.type == "cuda":
        best_global_info["cuda_device_name"] = torch.cuda.get_device_name(0)
    save_model_artifact(artifacts_dir, selected_model, selected_arch, rep_cfg, preprocessor, best_global_info)

    final_metrics = {
        "best_validation_auprc": float(best_global_info["validation_metrics"]["auprc"]),
        "selected_validation_auprc": float(best_global_info["validation_metrics"]["auprc"]),
        "best_model": best_global_info,
        "all_epochs": epoch_rows,
        "representation_smoke_test": smoke,
        "pretrained_loaded": bool(pretrained_summary.get("pretrained_loaded", False)),
        "pretrained_summary": pretrained_summary,
        "rinalmo_metadata": dict(rinalmo_metadata),
        "rinalmo_embedding_scaler_saved": False,
        "training_config": {
            "seed": SEED,
            ...
            "loss": "BCEWithLogitsLoss_no_class_weights",
            "scheduler": "ReduceLROnPlateau_mode_max_factor_0p5_patience_1",
            "single_model_no_ensemble": True,
        },
        "data": {
